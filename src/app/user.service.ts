...
export class UserService {
  ...
  update(uid, Email, Name) {
    return this.firestore.collection('users').doc(uid).set({ uid: uid, name: Name, email: Email });
  }
  // delete(id) {
  ...
  async loginStaff(email, password, staffNum) {
    ...
    this.firestore.collection('SingtelStaff').snapshotChanges().forEach(user => {
      ...
      user.forEach(userObject => {
        ...
        else if(email === userObject.payload.doc.data()["email"]  && i === 0){
          if (password !== userObject.payload.doc.data()["password"] || staffNum !== userObject.payload.doc.data()["staffNum"])
          {
            i = i + 1
            loading.dismiss();
    ...
    this.firestore.collection('Vendors').snapshotChanges().forEach(user => {
      ...
      user.forEach(userObject => {
        ...
        if (email === userObject.payload.doc.data()["email"] && password === userObject.payload.doc.data()["password"] && vendorNum === userObject.payload.doc.data()["vendorNum"]) {
          ...
          if (password !== userObject.payload.doc.data()["password"] || vendorNum !== userObject.payload.doc.data()["vendorNum"])
          {
            i = i + 1
            loading.dismiss();
            this.toast('please check your login credentials', 'danger')
  ...
  delete(uid: string): Promise<void> {
    ...
    return this.firestore.collection('users').doc(uid).delete().then(() => {
  ...
  deleteStaff(uid: string): Promise<void> {
    ...
    return this.firestore.collection('SingtelStaff').doc(uid).delete().then(() => {
      this.toast('account deleted!', 'success')
      this.router.navigate['/login'];
    })
  ...
  deleteVendor(uid: string): Promise<void> {
    return this.firestore.collection('Vendors').doc(uid).delete().then(() => {
      this.toast('account deleted!', 'success')
      this.router.navigate['/login'];
    })
